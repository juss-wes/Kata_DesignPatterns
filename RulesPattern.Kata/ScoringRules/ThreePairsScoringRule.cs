...
{
    public class ThreePairsScoringRule : IGreedScoringRule
    {
        ...
        public int ScoreDice(IEnumerable<DiceRoll> UnevaluatedDiceRolls)
        {
            ...
            if (UnevaluatedDiceRolls.Count() == 6 && UnevaluatedDiceRolls.GroupBy(dice => dice.NumberRolled).All(diceWithSameNumberRoll => diceWithSameNumberRoll.Count() == 2))
            {
                score += RuleScore;
